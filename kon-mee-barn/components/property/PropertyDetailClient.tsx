...
export function PropertyDetailClient({ property, userId, userRole, isFavorited: initialFav, similar = [] }: Props) {
      ...
      {similar.length > 0 && (
        <section className="mt-16 pt-10 border-t border-gray-200">
          <div className="flex items-end justify-between mb-6">
            <div>
              <h2 className="text-xl font-bold text-gray-900">{t.property.similarProperties}</h2>
              <p className="text-sm text-gray-500 mt-1">{t.property.similarDesc}</p>
            </div>
            <Link href={`/search?city=${encodeURIComponent(property.city)}`} className="text-sm text-primary hover:underline font-medium">
              {t.property.seeMore}
            </Link>
          </div>
