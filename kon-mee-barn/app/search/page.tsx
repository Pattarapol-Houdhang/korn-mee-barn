...
import { Loader2, LayoutGrid, List, Map as MapIcon, Search } from "lucide-react";
import { CITIES, PROPERTY_TYPES, TRANSACTION_TYPES } from "@/lib/utils";
import { useLanguage } from "@/components/shared/LanguageProvider";

const PropertiesMap = dynamic(() => import("@/components/search/PropertiesMap").then((m) => m.PropertiesMap), {
  ssr: false,
  loading: () => <div className="w-full h-[600px] rounded-xl border border-gray-200 bg-gray-100 animate-pulse" />,
});
...
function SearchContent() {
  ...
  const fetchProperties = useCallback(async () => {
    ...
    if (view === "map") params.set("limit", "100");
  ...
  }, [city, type, propertyType, bedrooms, maxPrice, sort, page, view]);
  ...
  const VIEW_BUTTONS: { value: ViewMode; icon: typeof LayoutGrid; label: string }[] = [
    { value: "grid", icon: LayoutGrid, label: t.search.gridView },
    { value: "list", icon: List, label: t.search.listView },
    { value: "map", icon: MapIcon, label: t.search.mapView },
  ];
    ...
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
        ...
        <div className="flex items-center gap-2 ml-auto">
          <label className="text-xs font-medium text-gray-600 whitespace-nowrap hidden sm:block">{t.search.sort}</label>
          ...
          <div className="flex gap-1 border border-gray-200 rounded-lg p-1">
            {VIEW_BUTTONS.map(({ value, icon: Icon, label }) => (
              <button
                key={value}
                onClick={() => setView(value)}
                title={label}
                className={`p-1.5 rounded transition-colors ${view === value ? "bg-primary text-primary-foreground" : "text-gray-500 hover:bg-gray-100"}`}
              >
      ...
      {loading ? (
        ...
        <PropertiesMap properties={properties as any} emptyLabel={t.search.noMapCoords} />
      ) : (
        <>
          ...
          <div className="flex items-center justify-center gap-3 mt-10">
            ...
            <span className="text-sm text-gray-500">
              {t.search.page} {page} {t.search.of} {Math.max(1, Math.ceil(total / 12))}
            </span>
